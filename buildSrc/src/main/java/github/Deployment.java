...
public class Deployment {
    ...
    public Response requestDeployment(Request request) throws Exception {
        ...
        try (CloseableHttpClient client = HttpClientCreator.create(username, password)) {
            HttpPost httpPost = new HttpPost("https://api.github.com/repos/AnySoftKeyboard/AnySoftKeyboard/deployments");
            httpPost.setHeader("Accept", "application/json");
            httpPost.setHeader("Content-type", "application/json");
            httpPost.setEntity(new StringEntity(requestJson, StandardCharsets.UTF_8));
            try (CloseableHttpResponse httpResponse = client.execute(httpPost)) {
                System.out.println("Response status: " + httpResponse.getStatusLine());
                return mGson.fromJson(new InputStreamReader(httpResponse.getEntity().getContent()), Response.class);
            }
        }
    }
    ...
    public static class Request {
        ...
        public Request(String ref, String task, boolean auto_merge, String environment, String description, List<String> required_contexts, RequestPayloadField payload) {
            this.ref = ref;
            this.task = task;
    ...
    public static class Response {
        ...
        public Response(String id, String sha, String ref, String task, RequestPayloadField payload, String environment) {
            this.id = id;
            this.sha = sha;
            this.ref = ref;
            this.task = task;
