...
./.github/actions/deploy/status-request.sh "${DEPLOYMET_ID}" "${DEPLOY_TARGET}" "in-progress" "${API_USER}" "${API_TOKEN}"
...
case "${DEPLOY_TARGET}" in
  ...
  app_promote_beta)
    DEPLOY_TASKS+=( "-DdeployChannel=beta" "ime:app:promoteReleaseArtifact" )
    ;;
  ...
  addons_promote_beta)
    DEPLOY_TASKS+=( "-DdeployChannel=beta" "promoteReleaseArtifact" "-x" "ime:app:promoteReleaseArtifact" )
    ;;
